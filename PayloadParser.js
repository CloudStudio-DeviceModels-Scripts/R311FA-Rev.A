//Test: 014F011D01000000000000 battery 2.9, status 01 - on
function parseUplink(device, payload) {
  ...
  // Store Velocity X
    if (decoded.velocityx != null) {
        ...
        if (sensor3 != null)
            sensor3.updateGenericSensorStatus(decoded.velocityx);
    };
     // Store Velocity Y
    if (decoded.velocityy != null) {
        ...
        if (sensor4 != null)
            sensor4.updateGenericSensorStatus(decoded.velocityy);
    };
    ...
    if (decoded.velocityz != null) {
        ...
        if (sensor5 != null)
            sensor5.updateGenericSensorStatus(decoded.velocityz);
    };
}
...
function Decoder(bytes, fport) {
    ...
    if (fport === 6) { // then its ReportDataCmd
        if (bytes[2] === 0x00) { // version report
            ...
            return decoded;
        } else { // device type 4F (R311FA), report type 01
